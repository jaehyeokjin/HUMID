...
PairTable_RLEUCG_WAT::PairTable_RLEUCG_WAT(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_WAT_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_WAT::~PairTable_RLEUCG_WAT()
{
...
void PairTable_RLEUCG_WAT::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  double exponential_factor = 0.508272776875881 * exp(-1.13054448043794 * cv);
  prob = 0.508272776875881 - exponential_factor;
  partial = 1.13054448043794 * exponential_factor;
}

/* ---------------------------------------------------------------------- */

int PairTable_RLEUCG_WAT::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_WAT::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_WAT::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_WAT::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_WAT::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_WAT::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_WAT::compute(int eflag, int vflag)
{
...
void PairTable_RLEUCG_WAT::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_WAT::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_WAT::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  if (eof == NULL) error->one(FLERR,"Unexpected end of RLEUCG_WAT state settings file");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        ...
        error->one(FLERR,"Unknown state assignment type for RLEUCG_WAT");
      }
...
void PairTable_RLEUCG_WAT::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_WAT::init_style()
{
  ...
  */
  ...
  else fprintf(screen, "Pair/RLEUCG_WAT Ensemble temperature is %lf.\n", T);
...
double PairTable_RLEUCG_WAT::init_one(int i, int j)
{
...
void PairTable_RLEUCG_WAT::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_WAT::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_WAT::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_WAT::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_WAT::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_WAT::null_table(Table *tb)
{
...
void PairTable_RLEUCG_WAT::free_table(Table *tb)
{
...
void PairTable_RLEUCG_WAT::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_WAT::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_WAT::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WAT::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WAT::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_WAT::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_WAT::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_WAT::extract(const char *str, int &dim)
{
