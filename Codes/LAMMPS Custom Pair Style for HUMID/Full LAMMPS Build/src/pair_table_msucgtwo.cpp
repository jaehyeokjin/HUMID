...
PairTable_MSUCGTWO::PairTable_MSUCGTWO(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_MSUCGTWO_NEIGH
  nmax = 0;
...
PairTable_MSUCGTWO::~PairTable_MSUCGTWO()
{
...
void PairTable_MSUCGTWO::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) {
...
int PairTable_MSUCGTWO::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_MSUCGTWO::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_MSUCGTWO::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_MSUCGTWO::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_MSUCGTWO::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_MSUCGTWO::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_MSUCGTWO::compute(int eflag, int vflag)
{
...
void PairTable_MSUCGTWO::allocate()
{
  allocated = 1;
...
void PairTable_MSUCGTWO::settings(int narg, char **arg)
{
...
void PairTable_MSUCGTWO::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  if (eof == NULL) error->one(FLERR,"Unexpected end of MSUCGTWO state settings file");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        ...
        error->one(FLERR,"Unknown state assignment type for MSUCGTWO");
      }
...
void PairTable_MSUCGTWO::coeff(int narg, char **arg)
{
...
void PairTable_MSUCGTWO::init_style()
{
  ...
  */
  ...
  else fprintf(screen, "Pair/MSUCGTWO Ensemble temperature is %lf.\n", T);
...
double PairTable_MSUCGTWO::init_one(int i, int j)
{
...
void PairTable_MSUCGTWO::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_MSUCGTWO::bcast_table(Table *tb)
{
...
void PairTable_MSUCGTWO::spline_table(Table *tb)
{
...
void PairTable_MSUCGTWO::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_MSUCGTWO::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_MSUCGTWO::null_table(Table *tb)
{
...
void PairTable_MSUCGTWO::free_table(Table *tb)
{
...
void PairTable_MSUCGTWO::spline(double *x, double *y, int n,
...
double PairTable_MSUCGTWO::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_MSUCGTWO::write_restart(FILE *fp)
{
...
void PairTable_MSUCGTWO::read_restart(FILE *fp)
{
...
void PairTable_MSUCGTWO::write_restart_settings(FILE *fp)
{
...
void PairTable_MSUCGTWO::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_MSUCGTWO::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_MSUCGTWO::extract(const char *str, int &dim)
{
