...
PairTable_RLEUCG_DIST_S::PairTable_RLEUCG_DIST_S(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_DIST_S_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_DIST_S::~PairTable_RLEUCG_DIST_S()
{
...
void PairTable_RLEUCG_DIST_S::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
    double tanh_factor = tanh((cv - cv_thresholds[type]) / (0.3 * cv_thresholds[type]));
    prob = 0.5 + 0.5 * tanh_factor;
    partial = 0.5 * (1.0 - tanh_factor * tanh_factor) / (0.3 * cv_thresholds[type]);
  }
  else {
...
int PairTable_RLEUCG_DIST_S::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_DIST_S::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_DIST_S::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_DIST_S::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_DIST_S::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_DIST_S::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_DIST_S::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  int itype_actual, jtype_actual;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,rsq_a,rsq_b,factor_lj,fraction,value,a,b;
  double delx1,delx2,dely1,dely2,delz1,delz2;
  int jj1,jj2,j1,j2,jtype1,jtype2;
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_dist_s:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_dist_s:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_dist_s:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_dist_s:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      if (distance_1 > distance_2) inumber_density = distance_2;
      else inumber_density = distance_1;
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      for (isubstate = 0; isubstate < n_states_per_type[itype_actual] - 1; isubstate++) {
        ...
        if (atom->tag[i] > 7){
          ...
          rsq_a = 100.0;
          rsq_b = 100.0;
          j1 = atom->map(atom->tag[i]+3);
          delx1 = xtmp - x[j1][0];
          dely1 = ytmp - x[j1][1];
          delz1 = ztmp - x[j1][2];
          rsq_a = delx1 * delx1 + dely1 * dely1 + delz1 * delz1;
          jtype1 = type[j1];
          j2 = atom->map(atom->tag[i]-3);
          delx2 = xtmp - x[j2][0];
          dely2 = ytmp - x[j2][1];
          delz2 = ztmp - x[j2][2];
          rsq_b = delx2 * delx2 + dely2 * dely2 + delz2 * delz2;
          jtype2 = type[j2];
...
void PairTable_RLEUCG_DIST_S::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_DIST_S::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_DIST_S::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_DIST_S::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_DIST_S::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_dist Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_DIST_S::init_one(int i, int j)
{
...
void PairTable_RLEUCG_DIST_S::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_DIST_S::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST_S::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST_S::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_DIST_S::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_DIST_S::null_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST_S::free_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST_S::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_DIST_S::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_DIST_S::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_DIST_S::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_DIST_S::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_DIST_S::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_DIST_S::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_DIST_S::extract(const char *str, int &dim)
{
