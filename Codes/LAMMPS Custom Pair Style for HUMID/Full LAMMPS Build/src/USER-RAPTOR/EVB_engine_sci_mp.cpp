...
void EVB_Engine::compute_sci_mp(int vflag)
{
  ...
  for(int i=0; i<ncomplex; i++) {
	...
	continue;
      ...
      if(!mp_verlet || mp_verlet->is_master==1) compute_diagonal(vflag);
...
void EVB_Engine::sci_finalize_mp(int vflag)
{
  ...
  for(int i=0; i<ncomplex; i++) {
    ...
    for(int j=1; j<nstate; j++) inter_energy += Cs[j]*Cs[parent[j]]*mtx->sci_e_offdiag[j-1];
    for(int j=0; j<nextra; j++) inter_energy += Cs[ex_i[j]]*Cs[ex_j[j]]*mtx->sci_e_extra[j];
  }
