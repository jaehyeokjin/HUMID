...
void EVB_EffPair::compute_q_eff_offdiag()
{
  double *Cs = evb_complex->Cs;
  double *Cs2 = evb_complex->Cs2;
  int *cplx_list = evb_complex->cplx_list;
...
void EVB_EffPair::compute_cplx(int cplx_id)
{
  ...
  for(int i=0; i<inum; i++) {
	...
	flag = true;
	int index = atp_index[type[atomi]];
	...
	flag = true;
	int index = atp_index[type[atomj]];
...
void EVB_EffPair::compute_exch(int cplx_id)
{
  ...
  for(int i=0; i<inum; i++) {
	...
	if (is_exch[atomi] && cplx_index[atomj]!=cplx_id) {
	  ...
	  ecoul += pre_ecoul_exch[id]*qq;
	}
	else if (cplx_index[atomi]!=cplx_id && is_exch[atomj]) {
	  qq = q_exch[atomj]*q[atomi];
	  ecoul += pre_ecoul_exch[id]*qq;
	}
      }
...
void EVB_EffPair::compute_finter(int vflag)
{
  ...
  for(int i=0; i<inum; i++) {
    ...
    EVB_Complex* cplx_A = evb_engine->all_complex[cplx_index[atomi]-1];

    for(int j=0; j<jnum; j++) {
      int atomj = jlist[j]  & NEIGHMASK;
      
      EVB_Complex* cplx_B = evb_engine->all_complex[cplx_index[atomj]-1];
      
      fpair = 0.0;
      
      if(!cut_lj[id]) {
	double* C2A = cplx_A->Cs2;
	double* C2B = cplx_B->Cs2;
	double nA = cplx_A->nstate;
	double nB = cplx_B->nstate;
	vdw1 = 0.0;
	vdw2 = 0.0;
	
	for(int i=0; i<nA; i++) for(int j=0; j<nB; j++) {
	    int itype = cplx_A->status[i].type[pos[atomi]];
	    int jtype = cplx_B->status[j].type[pos[atomj]];
	    double factor = C2A[i]*C2B[j];
...
void EVB_EffPair::compute_fenv(int vflag)
{
  ...
  double g_ewald = force->kspace->g_ewald;
  double qqrd2e = force->qqrd2e;
  double xi,yi,zi,dx,dy,dz,r2,r,t,erfc,grij,expm2,r2i,r6i,fpair;
  ...
  for(int i=0; i<inum; i++) {
    ...
    for(int j=0; j<jnum; j++) {
      int atomj = jlist[j];
      factor_coul = special_coul[sbmask(atomj)];
      factor_lj = special_lj[sbmask(atomj)];
      atomj &= NEIGHMASK;
	...
	for(int i=0; i<n; i++) {
	  int atype = cplx->status[i].type[pos[ia]]; 
  ...
  if(evb_engine->flag_DIAG_QEFF) {
    ...
    // Charges from only off-diagonals
    for(int i=0; i<evb_engine->ncomplex; i++) {
      evb_engine->evb_complex = evb_engine->all_complex[i];
      compute_q_eff_offdiag();
    }
    
    double cut = sqrt(cut_coulsq);
    double cut_cgis = 0.81 * cut; // The 0.81 could be an input parameter

    double A = -1.0 / (cut * cut * cut_cgis * (cut - cut_cgis));
    double B = -1.0 / (cut * cut * cut_cgis);
    double C =  1.0 / (cut * cut);
