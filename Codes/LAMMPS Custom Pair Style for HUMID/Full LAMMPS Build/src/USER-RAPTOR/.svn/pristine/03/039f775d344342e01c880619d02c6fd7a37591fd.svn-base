...
void FixColvars::one_time_init()
{
  ...
  // RAPTOR -- START
  EVB_GetFixObj(modify, &raptor);
  if(raptor) {
    if(me==0 && screen) fprintf(screen,"[MESSAGE] Found FixEVB Object.\n");

    // -- The cecs don't have image flags
    //if(unwrap_flag != 0) error->all(FLERR,"COLVARS + RAPTOR currently expects \"unwrap no\" for colvars fix.");
  } else if(me==0 && screen) fprintf(screen,"[MESSAGE] No FixEVB Object.\n");

  tagint tmp_tag = 0;
  for(int i=0; i<atom->nlocal; i++) 
    if(atom->tag[i] > tmp_tag) tmp_tag = atom->tag[i];
  
  // The tag for a cec will be max_lammps_tag + cplx_id*2 + cec_id
  // cec_id is 1 for cec object
  // cec_id is 2 for cec_v2 object
  MPI_Allreduce(&tmp_tag, &max_lammps_tag, 1, MPI_INT, MPI_MAX, world);
  // RAPTOR -- STOP
}
...
void FixColvars::setup(int vflag)
{
  ...
  nme=0;
  for (i=0; i < num_coords; ++i) {
    if(taglist[i] > max_lammps_tag) continue; // RAPTOR; skip if cec
  ...
  // RAPTOR -- START
  // Gonna be lazy here and just allocate memory for ncomplex*two more particles
  if(raptor) {
    int nc = raptor->Engine->ncomplex;
    nme+= nc*2;
  }
  // RAPTOR -- STOP
  ...
  if (me == 0) {
    ...
    for (i=0; i<num_coords; ++i) {
      ...
      // RAPTOR -- START
      if(taglist[i] > max_lammps_tag) {
	const tagint id = taglist[i] - max_lammps_tag;
	const int icplx = id / 2;
	const int icec = id - icplx * 2 - 1;

	if(raptor->Engine->rc_rank[icplx] != me) continue;

	EVB_GetCplx(raptor, icplx, &cplx);

	double cec[3];
	if(icec == 0) EVB_GetCEC(cplx, &cec[0]);
	else EVB_GetCECV2(cplx, &cec[0]);

	of[i].tag  = cd[i].tag  = taglist[i];
	of[i].type = cd[i].type = 0;
	...
	comm_buf[nme].tag  = taglist[i];
	comm_buf[nme].type = 0;
	  
	comm_buf[nme].x = cec[0];
	comm_buf[nme].y = cec[1];
	comm_buf[nme].z = cec[2];
...
void FixColvars::post_force(int vflag)
{
  ...
  for (int i=0; i < num_coords; ++i) {
    ...
    if(taglist[i] > max_lammps_tag) {
      ...
      if(icec == 0) EVB_PutCEC(cplx, &force_buf[3*i]);
      else EVB_PutCECV2(cplx, &force_buf[3*i]);
...
void FixColvars::end_of_step()
{
  if (store_forces) {
    ...
    // RAPTOR : Should probably check for cec particles and set old force to zero,
    // RAPTOR :  never going to do anything with old force.
