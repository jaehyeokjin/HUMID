...
#SN="10744"
#SN="11655"
#SN="12125"  # -- June 19, 2014
#SN="12794"  # -- Nov 25, 2014
SN="12990"  # -- Jan 27, 2015
...
if (test $1 = 1) then
  ...
  # ****************************************************
  echo "[$PKG_NAME] Check the SVN revision number of LAMMPS ... (${SN} is required)"
  
  ERROR_FLAG=""
  if [ -e '../.svn/entries' ]; then    
    _SN=`awk '{if(NR==4) print $1;}' ../.svn/entries`
    if [ "${_SN}" != "${SN}" ]; then
      ERROR_FLAG='set'
      echo "WARNING: Your revision number (${_SN}) is not compatible."
    fi  
  else    
    ERROR_FLAG='set'
    echo "WARNING: Can NOT find the SVN status. (You may not obtain the LAMMPS distribute from SVN)"  
  fi
  
  if [ "${ERROR_FLAG}" = "set" ]; then
    read -p "Do you want to continue? (yes/no) " REPLY
    
    if [ "${REPLY}" != "yes" ]; then exit; fi
  fi
