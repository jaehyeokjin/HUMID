...
void Verlet::run(int n)
{
  ...
  /* ---------------------------------------------------- */
  // AWGL: Check if we are doing mp_state
  /* ---------------------------------------------------- */
  int master_world    = 1;
  int flag_mp_state   = 0;
#ifdef STATE_DECOMP
  class FixEVB* fix_evb = NULL;
  MPI_Comm force_comm = 0;
  int group_root      = 0;
  for(int i=0; i<modify->nfix; i++) {
    if(strcmp(modify->fix[i]->style, "evb")==0) {
      fix_evb = (FixEVB*)(modify->fix[i]);
      flag_mp_state = fix_evb->Engine->flag_mp_state;
      break;
    }
  }
  if (flag_mp_state) {
    master_world = (universe->iworld == 0) ? 1 : 0;
    force_comm   = fix_evb->Engine->force_comm;
    group_root   = fix_evb->Engine->group_root;
  }
#endif
  ...
  for (int i = 0; i < n; i++) {
...
#ifdef STATE_DECOMP
      // Communicate updated positions and velocities
      int natom = atom->nlocal + atom->nghost;
      MPI_Bcast(&atom->x[0][0], 3*natom, MPI_DOUBLE, group_root, force_comm);
      MPI_Bcast(&atom->v[0][0], 3*natom, MPI_DOUBLE, group_root, force_comm);
#endif
    } else {
      modify->initial_integrate(vflag);
      if (n_post_integrate) modify->post_integrate();
    ...
    if (flag_mp_state) {
      ...
      MPI_Bcast(&nflag, 1, MPI_INT, 0, universe->uworld);
    }
    else {
      nflag = neighbor->decide();
    }
