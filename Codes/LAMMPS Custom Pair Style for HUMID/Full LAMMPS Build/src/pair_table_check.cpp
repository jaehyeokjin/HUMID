...
PairTable_CHECK::PairTable_CHECK(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_CHECK_NEIGH
  nmax = 0;
...
PairTable_CHECK::~PairTable_CHECK()
{
...
void PairTable_CHECK::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
    double tanh_factor = tanh((cv - cv_thresholds[type]) / 42.5);
    prob = 0.5 + 0.5 * tanh_factor;
    partial = 0.5 * (1.0 - tanh_factor * tanh_factor) / 42.5;
  }
  else {
...
int PairTable_CHECK::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_CHECK::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_CHECK::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_CHECK::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_CHECK::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_CHECK::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_CHECK::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/check:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/check:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/check:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/check:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    jlist = firstneigh[i];
    jnum = numneigh[i];
//    if (atom->tag[i] < 7)    fprintf(screen,"ID %d MAPPED %d Coord %f <===3===> %d MAPPED %d Coord %f \n", i, atom->tag[i], x[i][0], atom->tag[i]+3, atom->map(atom->tag[i]+3), x[atom->map(atom->tag[i]+3)][0]);
...
//	    fprintf(screen,"Atom (%d,%d) Tagged (%d,%d) Coord (%f %f %f) and (%f %f %f)\n",i, j, atom->tag[i], atom->tag[j], x[i][0],x[i][1], x[i][2],x[j][0],x[j][1],x[j][2]);
...
void PairTable_CHECK::settings(int narg, char **arg)
{
...
void PairTable_CHECK::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_CHECK::coeff(int narg, char **arg)
{
...
void PairTable_CHECK::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/check Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_CHECK::init_one(int i, int j)
{
...
void PairTable_CHECK::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_CHECK::bcast_table(Table *tb)
{
...
void PairTable_CHECK::spline_table(Table *tb)
{
...
void PairTable_CHECK::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_CHECK::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_CHECK::null_table(Table *tb)
{
...
void PairTable_CHECK::free_table(Table *tb)
{
...
void PairTable_CHECK::spline(double *x, double *y, int n,
...
double PairTable_CHECK::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_CHECK::write_restart(FILE *fp)
{
...
void PairTable_CHECK::read_restart(FILE *fp)
{
...
void PairTable_CHECK::write_restart_settings(FILE *fp)
{
...
void PairTable_CHECK::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_CHECK::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_CHECK::extract(const char *str, int &dim)
{
