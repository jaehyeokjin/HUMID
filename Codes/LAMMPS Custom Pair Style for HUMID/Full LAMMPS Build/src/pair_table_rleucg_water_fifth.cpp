...
PairTable_RLEUCG_WATER_FIFTH::PairTable_RLEUCG_WATER_FIFTH(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_WATER_FIFTH_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_WATER_FIFTH::~PairTable_RLEUCG_WATER_FIFTH()
{
...
void PairTable_RLEUCG_WATER_FIFTH::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial, double &cv_val) 
{
  if (type == 1){
    double tanh_factor = tanh((cv - cv_thresholds[type]) / (0.1*cv_thresholds[type])); // * cv_thresholds[type]));
    prob = 0.5 + 0.5 * tanh_factor;
    partial = 0.5 * (1.0 - tanh_factor * tanh_factor) / (0.1*cv_thresholds[type]); // * cv_thresholds[type]);
    cv_val = cv;
  }
  else {
...
int PairTable_RLEUCG_WATER_FIFTH::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_WATER_FIFTH::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_WATER_FIFTH::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_WATER_FIFTH::unpack_forward_comm(int n, int first, double *buf)
{
...
void PairTable_RLEUCG_WATER_FIFTH::compute(int eflag, int vflag)
{
...
// End of CHOA routine
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_water_fifth:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_water_fifth:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_water_fifth:substate_probability_force");
    memory->grow(per_particle_cv, nall,  "pair/rleucg_water_fifth:per_particle_cv");    
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_water_fifth:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      if (jnum > maxneigh) {
        ...
        memory->destroy(nearest);
        maxneigh = jnum;
        memory->create(distsq,maxneigh,"pair/rleucg_water_fifth:distsq");
        memory->create(nearest,maxneigh,"pair/rleucg_water_fifth:nearest");
      }
      ...
      dely = ytmp - x[j][1];
      delz = ztmp - x[j][2];
      usum = delx*delx + dely*dely + delz*delz;
      vsum = sqrt(usum);
...
void PairTable_RLEUCG_WATER_FIFTH::select2(int k, int n, double *arr, int *iarr)
{
...
void PairTable_RLEUCG_WATER_FIFTH::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_WATER_FIFTH::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER_FIFTH::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_WATER_FIFTH::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER_FIFTH::init_style()
{
...
double PairTable_RLEUCG_WATER_FIFTH::init_one(int i, int j)
{
...
void PairTable_RLEUCG_WATER_FIFTH::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_WATER_FIFTH::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_FIFTH::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_FIFTH::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_WATER_FIFTH::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_WATER_FIFTH::null_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_FIFTH::free_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_FIFTH::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_WATER_FIFTH::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_WATER_FIFTH::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_FIFTH::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_FIFTH::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_FIFTH::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_WATER_FIFTH::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_WATER_FIFTH::extract(const char *str, int &dim)
{
