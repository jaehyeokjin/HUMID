...
PairTable_com_corr::PairTable_com_corr(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairTable_com_corr::~PairTable_com_corr()
{
...
void PairTable_com_corr::compute(int eflag, int vflag)
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (itype != jtype){
        ...
        if (rsqcom < cutsq[3][3]) {
          ...
          f[i][1] += fpair*dcomy;
          f[i][2] += fpair*dcomz;
          fprintf(screen, "%lf, %lf, %lf \n", fpair*dcomx,fpair*dcomy,fpair*dcomz);
          ...
          if (evflag) ev_tally(i,j,nlocal,newton_pair,
                               evdwl,0.0,fpair,dcomx, dcomy, dcomz);
        }
      }
...
void PairTable_com_corr::allocate()
{
  allocated = 1;
...
void PairTable_com_corr::settings(int narg, char **arg)
{
...
void PairTable_com_corr::coeff(int narg, char **arg)
{
...
double PairTable_com_corr::init_one(int i, int j)
{
...
void PairTable_com_corr::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_com_corr::bcast_table(Table *tb)
{
...
void PairTable_com_corr::spline_table(Table *tb)
{
...
void PairTable_com_corr::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_com_corr::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_com_corr::null_table(Table *tb)
{
...
void PairTable_com_corr::free_table(Table *tb)
{
...
void PairTable_com_corr::spline(double *x, double *y, int n,
...
double PairTable_com_corr::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_com_corr::write_restart(FILE *fp)
{
...
void PairTable_com_corr::read_restart(FILE *fp)
{
...
void PairTable_com_corr::write_restart_settings(FILE *fp)
{
...
void PairTable_com_corr::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_com_corr::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_com_corr::extract(const char *str, int &dim)
{
