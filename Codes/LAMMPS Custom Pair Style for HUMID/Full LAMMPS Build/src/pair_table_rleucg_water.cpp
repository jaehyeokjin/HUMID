...
PairTable_RLEUCG_WATER::PairTable_RLEUCG_WATER(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_WATER_NEIGH
  nmax = 0;
  ...
  per_particle_cv = NULL;
  state_params_allocated = 0;

  comm_reverse = 3;
  comm_forward = 3;

// Added feature from the compute_hexorder_atom routine
  nnn = 4;
  nmax = 0;
  maxneigh = 0;
  distsq = NULL;
...
PairTable_RLEUCG_WATER::~PairTable_RLEUCG_WATER()
{
...
void PairTable_RLEUCG_WATER::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial, double &cv_val) 
{
  if (type == 1){
    ...
    double tanh_factor = tanh((check_factor - cv_thresholds[type]) / (0.15)); // * cv_thresholds[type]));
    prob = 0.5 + 0.5 * tanh_factor;
    partial = 0.5 * (1.0 - tanh_factor * tanh_factor) / (0.15); // * cv_thresholds[type]);
    cv_val = check_factor;
  }
  else {
    error->one(FLERR,"Declare type is non-existent. Unwieldy calculation have been performed");
  }
}

/* ---------------------------------------------------------------------- */

int PairTable_RLEUCG_WATER::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_WATER::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_WATER::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
//    buf[m++] = per_particle_cv[j];
  }
  return m;
}

void PairTable_RLEUCG_WATER::unpack_forward_comm(int n, int first, double *buf)
{
...
void PairTable_RLEUCG_WATER::compute(int eflag, int vflag)
{
  int i,j,ii,jj,k,kk,inum,jnum,knum,itype,jtype,ktype,itable;
  int itype_actual, jtype_actual;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double delx1,dely1,delz1, u;
  double ij_pair, ik_pair;
  double rsq,rsq1,factor_lj,fraction,value,a,b;
  double distance,distance1,inumber_density,jnumber_density, cv_force;
...
// End of CHOA routine
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_water:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_water:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_water:substate_probability_force");
    memory->grow(per_particle_cv, nall,  "pair/rleucg_water:per_particle_cv");    
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_water:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      if (jnum > maxneigh) {
        ...
        memory->destroy(nearest);
        maxneigh = jnum;
        memory->create(distsq,maxneigh,"pair/rleucg_water:distsq");
        memory->create(nearest,maxneigh,"pair/rleucg_water:nearest");
      }
      ...
      for (jj = 0; jj < ncount-1; jj++) {
        ...
        for (kk = jj+1; kk < ncount; kk++) {
          ...
          dely1 = ytmp - x[k][1];
          delz1 = ztmp - x[k][2];
          u = calc_qn_complex(delx, dely, delz, delx1, dely1, delz1);
          usum += (u+1.0/3.0)*(u+1.0/3.0);
        }
      }
      vsum = 1.0-3.0*usum/8.0;
      threshold_prob_and_partial_from_cv(itype_actual, vsum, substate_probability[i][0], substate_probability_partial[i][0], per_particle_cv[i]);
      //fprintf(screen,"I: %d Value: %lf, %lf\n", atom->tag[i], vsum, per_particle_cv[i]);
      //fprintf(screen, "%d %lf\n", atom->tag[i], vsum);
    }
    else {
      ...
      fprintf(screen, " ERROR ERROR ERROR\n"); 
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
	    ...
	    // fprintf(screen,"(i,j) = (%d,%d) has (%d,%d)type\n",atom->tag[i], atom->tag[j], alpha, beta);
            ...
            if (n_states_per_type[itype_actual] > 1) {
              if (isubstate < n_states_per_type[itype_actual] - 1) {
//                fprintf(screen,"%d %d First loop %lf %lf\n",i, isubstate,betaprob,evdwl);
...
//                  fprintf(screen,"%d %dSecond loop %lf %lf\n",i, isubstate,betaprob,evdwl);
      ...
      //fprintf(screen,"%d======== %lf\n",i, substate_probability_force[i][0]);
      }
    }
  }
  comm->forward_comm_pair(this);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      for (isubstate = 0; isubstate < n_states_per_type[itype_actual] - 1; isubstate++) {
        ...
        //fprintf(screen,"Third loop: %d, %lf\n", i, substate_probability_force[i][0]);
        ...
        for (jj = 0; jj < ncount-1; jj++) {
          ...
          for (kk= jj+1; kk < ncount; kk++) {
            ...
            // We first need the angle between these three. Using same routine as of the first loop
            // First it is ij pair
            distance = sqrt(rsq);
            distance1 = sqrt(rsq1);
            u = (delx*delx1+dely*dely1+delz*delz1)/(distance*distance1);
//            fprintf(screen,"%d %d %d %lf %lf %lf\n", atom->tag[i], atom->tag[j], atom->tag[k], distance, distance1, per_particle_cv[i]);
            fpair = cv_force * 6.0 * (u+1.0/3.0)/8.0;
            pair_force = fpair;
            substate_cv_backforce[i][0] += fpair * ((delx1/distance1 - u*delx/distance)/distance+(delx/distance-u*delx1/distance1)/distance1);
            substate_cv_backforce[i][1] += fpair * ((dely1/distance1 - u*dely/distance)/distance+(dely/distance-u*dely1/distance1)/distance1);
            substate_cv_backforce[i][2] += fpair * ((delz1/distance1 - u*delz/distance)/distance+(delz/distance-u*delz1/distance1)/distance1);
            substate_cv_backforce[j][0] -= fpair * (delx1/distance1 - u*delx/distance)/distance;
            substate_cv_backforce[j][1] -= fpair * (dely1/distance1 - u*dely/distance)/distance;
            substate_cv_backforce[j][2] -= fpair * (delz1/distance1 - u*delz/distance)/distance;
            substate_cv_backforce[k][0] -= fpair * (delx/distance - u*delx1/distance1)/distance1;
            substate_cv_backforce[k][1] -= fpair * (dely/distance - u*dely1/distance1)/distance1;
            substate_cv_backforce[k][2] -= fpair * (delz/distance - u*delz1/distance1)/distance1;
            ev_tally(i,j,nlocal,newton_pair,0.0,0.0,pair_force,delx,dely,delz);
            ev_tally(i,k,nlocal,newton_pair,0.0,0.0,pair_force,delx1,dely1,delz1);
          }
        }
      }
    }
...
// From CHOA routine
double PairTable_RLEUCG_WATER::calc_qn_complex(double delx, double dely, double delz, double delx1, double dely1, double delz1) {
  double answer = 0.0;
  double dist_1 = sqrt(delx*delx+dely*dely+delz*delz);
  double dist_2 = sqrt(delx1*delx1+dely1*dely1+delz1*delz1);
  double dot_pdt = delx*delx1+dely*dely1+delz*delz1;
  answer = dot_pdt/(dist_1*dist_2);
  return(answer);
}
...
void PairTable_RLEUCG_WATER::select2(int k, int n, double *arr, int *iarr)
{
...
void PairTable_RLEUCG_WATER::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_WATER::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  comm_forward = 4 * (max_states_per_type - 1);
...
void PairTable_RLEUCG_WATER::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_interface Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_WATER::init_one(int i, int j)
{
...
void PairTable_RLEUCG_WATER::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_WATER::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_WATER::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_WATER::null_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER::free_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_WATER::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_WATER::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_WATER::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_WATER::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_WATER::extract(const char *str, int &dim)
{
