...
PairTable_RLEUCG_THIOL::PairTable_RLEUCG_THIOL(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_THIOL_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_THIOL::~PairTable_RLEUCG_THIOL()
{
...
void PairTable_RLEUCG_THIOL::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 2){
    double exponential_factor = 0.395783347713344 * exp(-0.323873947953137 * cv);
    prob =  (1.0-0.354451786102697) - exponential_factor;
    partial = 0.323873947953137 * exponential_factor;
  }
/*
  else if (type == 1){
...
int PairTable_RLEUCG_THIOL::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_THIOL::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_THIOL::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_THIOL::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_THIOL::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_THIOL::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_THIOL::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_thiol:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_thiol:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_thiol:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_thiol:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        jtype = type[j];
//         if ((itype == 1 && jtype == 3) || (itype == 3 && jtype == 3)){
        if (itype == 1 && jtype == 2){
          delx = xtmp - x[j][0];
          dely = ytmp - x[j][1];
          delz = ztmp - x[j][2];
...
void PairTable_RLEUCG_THIOL::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_THIOL::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_THIOL::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_THIOL::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_THIOL::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_thiol Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_THIOL::init_one(int i, int j)
{
...
void PairTable_RLEUCG_THIOL::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_THIOL::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_THIOL::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_THIOL::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_THIOL::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_THIOL::null_table(Table *tb)
{
...
void PairTable_RLEUCG_THIOL::free_table(Table *tb)
{
...
void PairTable_RLEUCG_THIOL::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_THIOL::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_THIOL::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_THIOL::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_THIOL::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_THIOL::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_THIOL::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_THIOL::extract(const char *str, int &dim)
{
