...
PairTable_RLEUCG_WATER_DD::PairTable_RLEUCG_WATER_DD(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_WATER_DD_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_WATER_DD::~PairTable_RLEUCG_WATER_DD()
{
  ...
  if (state_params_allocated) {
    ...
    memory->destroy(threshold_inner);
  }
}

/* ---------------------------------------------------------------------- */

void PairTable_RLEUCG_WATER_DD::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTable_RLEUCG_WATER_DD::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_WATER_DD::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_WATER_DD::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_WATER_DD::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_WATER_DD::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return (0.5 * (1.0 - tanh_factor));
}

double PairTable_RLEUCG_WATER_DD::compute_proximity_function_inner(int type, double distance) {
  double tanh_innerf = tanh((distance - threshold_inner[type]) / (0.1 * threshold_inner[type]));
  return (0.5 * (1.0 - tanh_innerf));
}

double PairTable_RLEUCG_WATER_DD::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return ((0.5 * (1.0 - tanh_factor * tanh_factor) / (0.1 * threshold_radii[type])));
}

double PairTable_RLEUCG_WATER_DD::compute_proximity_function_der_inner(int type, double distance) {
  double tanh_innerf = tanh((distance - threshold_inner[type]) / (0.1 * threshold_inner[type]));
  return ((0.5 * (1.0 - tanh_innerf * tanh_innerf) / (0.1 * threshold_inner[type])));
}

/* ---------------------------------------------------------------------- */

void PairTable_RLEUCG_WATER_DD::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  int itype_actual, jtype_actual;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,factor_lj,fraction,value,a,b;
  double ratio_r = threshold_radii[1]/threshold_inner[1];
  double distance,inumber_density,jnumber_density, cv_force;
  double inumber_inner, derivate_inner;
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_water_dd:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_water_dd:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_water_dd:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_water_dd:substate_cv_backforce");
  }
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if ((itype == 1 && jtype == 1)){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            inumber_inner += compute_proximity_function_inner(itype_actual, distance);
          }
        }
      }
      inumber_density = inumber_density - inumber_inner * ratio_r * ratio_r * ratio_r;
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      for (isubstate = 0; isubstate < n_states_per_type[itype_actual] - 1; isubstate++) {
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          if (rsq < cutsq[itype][jtype]) {
            distance = sqrt(rsq);
            derivate_inner = compute_proximity_function_der(itype_actual, distance) - compute_proximity_function_der_inner(itype_actual, distance);
            fpair = cv_force * derivate_inner/ distance;
            pair_force = fpair;
...
void PairTable_RLEUCG_WATER_DD::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_WATER_DD::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER_DD::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  memory->create(threshold_inner, n_actual_types + 1, "pair:n_states_per_type");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        eof = fgets(line, MAXLINE, fp);
        if (eof == NULL) error->one(FLERR,"Unexpected end of RLEUCG state settings file");
        sscanf(line, "%lg %lg %lg", &cv_thresholds[i], &threshold_radii[i], &threshold_inner[i]);
      } else {
...
void PairTable_RLEUCG_WATER_DD::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_WATER_DD::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_water_dd Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_WATER_DD::init_one(int i, int j)
{
...
void PairTable_RLEUCG_WATER_DD::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_WATER_DD::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_DD::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_DD::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_WATER_DD::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_WATER_DD::null_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_DD::free_table(Table *tb)
{
...
void PairTable_RLEUCG_WATER_DD::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_WATER_DD::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_WATER_DD::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_DD::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_DD::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_WATER_DD::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_WATER_DD::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_WATER_DD::extract(const char *str, int &dim)
{
