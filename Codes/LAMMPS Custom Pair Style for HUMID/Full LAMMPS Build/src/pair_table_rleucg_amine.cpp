...
PairTable_RLEUCG_AMINE::PairTable_RLEUCG_AMINE(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_AMINE_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_AMINE::~PairTable_RLEUCG_AMINE()
{
...
void PairTable_RLEUCG_AMINE::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 2){
    double exponential_factor = 0.261114182550898 * exp(-0.851288145047532 * cv);
    prob =  (1.0-0.484150568971338) - exponential_factor;
    partial = 0.851288145047532 * exponential_factor;
  }
  else if (type == 1){
...
int PairTable_RLEUCG_AMINE::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_AMINE::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_AMINE::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_AMINE::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_AMINE::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_AMINE::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_AMINE::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_amine:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_amine:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_amine:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_amine:substate_cv_backforce");
  }
...
void PairTable_RLEUCG_AMINE::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_AMINE::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_AMINE::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_AMINE::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_AMINE::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_amine Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_AMINE::init_one(int i, int j)
{
...
void PairTable_RLEUCG_AMINE::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_AMINE::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_AMINE::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_AMINE::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_AMINE::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_AMINE::null_table(Table *tb)
{
...
void PairTable_RLEUCG_AMINE::free_table(Table *tb)
{
...
void PairTable_RLEUCG_AMINE::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_AMINE::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_AMINE::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_AMINE::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_AMINE::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_AMINE::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_AMINE::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_AMINE::extract(const char *str, int &dim)
{
