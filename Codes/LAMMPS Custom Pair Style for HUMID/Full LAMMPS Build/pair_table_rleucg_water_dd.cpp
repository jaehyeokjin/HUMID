...
double PairTable_RLEUCG_WATER_DD::compute_proximity_function(int type, double distance) {
  ...
  double ratio_r = threshold_radii[type]/threshold_inner[type];
  return ((0.5 * (1.0 - tanh_factor)) - ((0.5 * (1.0 - tanh_innerf)))*(ratio_r*ratio_r*ratio_r));
}
...
double PairTable_RLEUCG_WATER_DD::compute_proximity_function_der(int type, double distance) {
  ...
  double tanh_innerf = tanh((distance - threshold_inner[type]) / (0.1 * threshold_inner[type]));
  double ratio_r = threshold_radii[type]/threshold_inner[type];
  return ((0.5 * (1.0 - tanh_factor * tanh_factor) / (0.1 * threshold_radii[type])) - (0.5 * (1.0 - tanh_innerf * tanh_innerf) / (0.1 * threshold_inner[type]))*(ratio_r*ratio_r*ratio_r));
}
