...
double PairMSUCG_NEIGH::P(int type, int state, double *x, double *f, double w, double *uval, int single)
{
  ...
  if(type==1)
  {
    ...
    double factor_p = 1.0/(0.0004 * p_constant * sigma_cutoff) * (1.0 - pow(tanh((w - p_constant)/(0.1 * p_constant)), 2.0));
...
/* */
  ...
  /* Subforce routine */
  double *subforce_1=(double*)malloc(sizeof(double)*3);
  double *subforce_2=(double*)malloc(sizeof(double)*3);
  double *subforce_3=(double*)malloc(sizeof(double)*3);
  double *subforce_4=(double*)malloc(sizeof(double)*3);
  /* Final force routine */
  double *totalforce=(double*)malloc(sizeof(double)*3);
  double pair_force = (double*)malloc(sizeof(double)*1); // For updating in the ev_tally routine
...
/* First subforce part from one particle interaction */
    ...
    while(alpha != 0)
    {
      ...
      subforce_1[0] = kT * log(p) * ff[0];
      subforce_1[1] = kT * log(p) * ff[1];
      subforce_1[2] = kT * log(p) * ff[2];
/* End of first subforce part from one particle interaction */
    ...
    /* i and j are in the reduced cutoff distance: neighbor */
	    ...
	    while(alpha !=0)
	    {
          ...
          /* First subforce part from two particle interaction */
          	...
          	double sech_one = 1.0 - pow(tanh((sqrt(rsq) - sigma_cutoff)/(0.001 * sigma_cutoff)),2.0);
          	subforce_1[0] += kT * log(pj_one) * ffj_one[0] * sech_one * delx/sqrt(rsq);
          	subforce_1[1] += kT * log(pj_one) * ffj_one[1] * sech_one * dely/sqrt(rsq);
          	subforce_1[2] += kT * log(pj_one) * ffj_one[2] * sech_one * delz/sqrt(rsq);
          ...
          /* End of first subforce part coupled with particle j */
	        while(beta != 0)
	        {
            	...
            	subforce_2[0] -= fpair * delx;
	            subforce_2[1] -= fpair * dely;
	            subforce_2[2] -= fpair * delz;
	            ...
	            /* Third subforce term */
	            subforce_3[0] -= ffi[0] * pj * evdwl - ffj[0] * pi * evdwl;
	            subforce_3[1] -= ffi[1] * pj * evdwl - ffj[1] * pi * evdwl;
	            subforce_3[2] -= ffi[2] * pj * evdwl - ffj[2] * pi * evdwl;
		    ...
		    pair_force += (-1.0 * pi_reduce * ffi_reduce[0] * pj * evdwl - pj_reduce * ffj_reduce[0] * pi * evdwl)
        ...
        if (rsqjk < 0.25*cutsq[jtype][ktype]) 
        {
	    			...
	    			int beta  = ktype;
				        ...
				        while(alpha !=0)
          				...
          				if(alpha == 2){
	        				...
	        				{
            				double ffj[3], ffk[3];
            				double pj = P(jtype, alpha, x[j], ffj, W[j], dW[j],1);
            				double pk = P(ktype, beta, x[k], ffk, W[k], dW[k],1);
            				double sech_one_j = 1.0 - pow(tanh((sqrt(rsqjk) - sigma_cutoff)/(0.001 * sigma_cutoff)),2.0);
            				double sech_one_k = 1.0 - pow(tanh((sqrt(rsqik) - sigma_cutoff)/(0.001 * sigma_cutoff)),2.0);

	          				forcelj = r6jkinv * (lj1[alpha][beta]*r6jkinv - lj2[alpha][beta]);
            				ffactor_jk = factor_lj* forcelj*r2jkinv;
                      
                     		subforce_4[0] = ffactor_jk * (pj * ffk[0] * sech_one_k * delikx/sqrt(rsqik) + pk * ffj[0] * sech_one_j * delx/sqrt(rsq));
                     		subforce_4[1] = ffactor_jk * (pj * ffk[1] * sech_one_k * deliky/sqrt(rsqik) + pk * ffj[1] * sech_one_j * dely/sqrt(rsq));
                     		subforce_4[2] = ffactor_jk * (pj * ffk[2] * sech_one_k * delikz/sqrt(rsqik) + pk * ffj[2] * sech_one_j * delz/sqrt(rsq));
		     				pair_force = ffactor_jk * (pj * ffk[0] * sech_one_k/sqrt(rsqik) + pk * ffj[0] * sech_one_j/sqrt(rsq));
            				if (newton_pair || j < nlocal)
            				{
              					f[j][0] -= delx * fpair - ffi[0] * pj * evdwl;
              			 		f[j][1] -= dely * fpair - ffi[1] * pj * evdwl;
              ...
              while(alpha != 0)
              {
                ...
                while(beta != 0)
                {
                  ...
                  subforce_4[0] = ffactor_jk * (pj * ffk[0] * sech_one_k * delikx/sqrt(rsqik));
                  subforce_4[1] = ffactor_jk * (pj * ffk[1] * sech_one_k * deliky/sqrt(rsqik));
                  subforce_4[2] = ffactor_jk * (pj * ffk[2] * sech_one_k * delikz/sqrt(rsqik));
		  		  pair_force += ffactor_jk * (pj * ffk[0] * sech_one_k/sqrt(rsqik));
                  if (newton_pair || j < nlocal)
                  {
                    f[j][0] -= delx * fpair - ffi[0] * pj * evdwl;
                    f[j][1] -= dely * fpair - ffi[1] * pj * evdwl;
      ...
      /* End of k loop */
      totalforce[0] = subforce_1[0]+subforce_2[0]+subforce_3[0]+subforce_4[0];
      totalforce[1] = subforce_1[1]+subforce_2[1]+subforce_3[1]+subforce_4[1];
      totalforce[2] = subforce_1[2]+subforce_2[2]+subforce_3[2]+subforce_4[2];
      f[i][0] += totalforce[0];
      f[i][1] += totalforce[1];
      f[i][2] += totalforce[2];
