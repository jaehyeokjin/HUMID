...
/* ----------------------------------------------------------------------
   ...
   This is derived from the table, sw, and lj/cut/coul/long pairstyles.
------------------------------------------------------------------------- */
...
PairTableSWLJCutCoulLong::PairTableSWLJCutCoulLong(LAMMPS *lmp) : Pair(lmp)
{
  ...
  sw_scale_epsilon = NULL;
  sw_scale_biga    = NULL;
  sw_scale_lambda  = NULL;
}

/* ---------------------------------------------------------------------- */

PairTableSWLJCutCoulLong::~PairTableSWLJCutCoulLong()
{
  for (int m = 0; m < ntables; m++) free_table(&tables[m]);
  memory->sfree(tables);

  if(elements) for(int i=0; i<nelements; i++) delete [] elements[i];
  delete [] elements;
  ...
  if (allocated) {
    ...
    // sw
    memory->destroy(sw_scale_epsilon);
    memory->destroy(sw_scale_biga);
    memory->destroy(sw_scale_lambda);
...
void PairTableSWLJCutCoulLong::compute(int eflag, int vflag)
{ 
  int i,j,k,ii,jj,kk,inum,jnum,jnumm1,itype,jtype,itable;
  int itag,jtag,ijparam,ikparam,ijkparam;
  int itypeM,jtypeM,ktypeM;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	// Add contribution from tabulated potential
      ...
      jtypeM = map[jtype];
      if(jtypeM == -1) continue;

      ijparam = elem2param[itypeM][jtypeM][jtypeM];
    ...
    for (jj = 0; jj < jnumm1; jj++) {
      ...
      for (kk = jj+1; kk < jnum; kk++) {
    	...
    	k &= NEIGHMASK;
    	ktypeM = map[type[k]];
    	if(ktypeM == -1) continue;

    	ikparam = elem2param[itypeM][ktypeM][ktypeM];
    	ijkparam = elem2param[itypeM][jtypeM][ktypeM];
...
void PairTableSWLJCutCoulLong::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
  ...
  memory->create(sw_scale_epsilon, n+1, n+1, "pair:sw_scale_epsilon");
  memory->create(sw_scale_biga, n+1, n+1, "pair:sw_scale_biga");
  memory->create(sw_scale_lambda, n+1, n+1, "pair:sw_scale_lambda");
  for (int i = 1; i <= n; i++)
    for (int j = i; j <= n; j++) {
      setflag[i][j] = 0;
      tabflag[i][j] = 0;
      tab_scale[i][j] = 1.0;
      sw_scale_epsilon[i][j] = sw_scale_epsilon[j][i] = 1.0;
      sw_scale_biga[i][j]    = sw_scale_biga[j][i]    = 1.0;
      sw_scale_lambda[i][j]  = sw_scale_lambda[j][i]  = 1.0;
    }
...
void PairTableSWLJCutCoulLong::settings(int narg, char **arg)
{
  int indx;
...
void PairTableSWLJCutCoulLong::coeff(int narg, char **arg)
{
  int i,j,n;

  if( (strcmp(arg[2],"lj")  == 0) && (narg < 5 || narg > 6) ) error->one(FLERR,"Incorrect args for lj pair coefficients");
  if( (strcmp(arg[2],"tab") == 0) && (narg < 5 || narg > 6) ) error->one(FLERR,"Incorrect args for tab pair coefficients");
  if( (strcmp(arg[2],"sw")  == 0) && (narg != 3 + atom->ntypes + 1) ) error->one(FLERR,"Incorrect args for sw pair coefficients");
...
void PairTableSWLJCutCoulLong::init_style()
{
  if (!atom->q_flag)
    error->one(FLERR,"Pair style table/sw/lj/cut/coul/long requires atom attribute q");

  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style table/sw/lj/cut/coul/long requires atom IDs");
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style table/sw/lj/cut/coul/long requires newton pair on");
...
double PairTableSWLJCutCoulLong::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if(nparams > 0) {
    for (int m = 0; m < nparams; m++) {
      // scaling factors updated from fix_adapt
      params[m].epsilon *= sw_scale_epsilon[1][1];
      params[m].lambda  *= sw_scale_lambda[1][1];
      params[m].biga    *= sw_scale_biga[1][1];
...
void PairTableSWLJCutCoulLong::init_tables()
{
  int masklo,maskhi;
...
void PairTableSWLJCutCoulLong::write_restart(FILE *fp)
{
...
void PairTableSWLJCutCoulLong::read_restart(FILE *fp)
{
...
void PairTableSWLJCutCoulLong::write_restart_settings(FILE *fp)
{
...
void PairTableSWLJCutCoulLong::read_restart_settings(FILE *fp)
{
...
void PairTableSWLJCutCoulLong::free_tables()
{
...
void *PairTableSWLJCutCoulLong::extract(const char *str, int &dim)
{
  ...
  // lj/cut
  dim = 2;
  if (strcmp(str,"lj_epsilon") == 0) return (void *) epsilon;
  ...
  // sw
  dim = 2;
  if (strcmp(str,"sw_epsilon") == 0) return (void *) sw_scale_epsilon;
  if (strcmp(str,"sw_biga")    == 0) return (void *) sw_scale_biga;
  if (strcmp(str,"sw_lambda")  == 0) return (void *) sw_scale_lambda;

  if(screen) {
    fprintf(screen,"\n\nSupported extract() arguments for pair_style table/sw/lj/cut/coul/long\n");
    fprintf(screen,"cut_coul\n");
    fprintf(screen,"lj_epsilon\n");
    fprintf(screen,"tab_scale\n");
    fprintf(screen,"sw_epsilon, sw_biga, sw_lambda\n\n");
  }
...
void PairTableSWLJCutCoulLong::free_table(Table *tb)
{
...
void PairTableSWLJCutCoulLong::null_table(Table *tb)
{
...
void PairTableSWLJCutCoulLong::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTableSWLJCutCoulLong::bcast_table(Table *tb)
{
...
void PairTableSWLJCutCoulLong::spline_table(Table *tb)
{
...
void PairTableSWLJCutCoulLong::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = 0;
  tb->fpflag = 0;
...
void PairTableSWLJCutCoulLong::compute_table(Table *tb)
{
  int tlm1 = tablength - 1;
...
void PairTableSWLJCutCoulLong::spline(double *x, double *y, int n,
...
double PairTableSWLJCutCoulLong::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTableSWLJCutCoulLong::read_file(char *file)
{
...
void PairTableSWLJCutCoulLong::setup()
{
  int i,j,k,m,n;
  double rtmp;
...
void PairTableSWLJCutCoulLong::twobody(Param *param, double rsq, double &fforce,
...
void PairTableSWLJCutCoulLong::threebody(Param *paramij, Param *paramik, Param *paramijk,
...
double PairTableSWLJCutCoulLong::single(int i, int j, int itype, int jtype,
				      ...
				      double dum1, double & dum2)
{
  ...
  if( tabflag[itype][jtype] ) {
	...
	fraction = (rsq - tb->rsq[itable]) * tb->invdelta;
	value = tb->f[itable] + fraction*tb->df[itable];
	fpair = factor_lj * value * scale;
      }
      
      phi = tb->e[itable] + fraction*tb->de[itable];
      
      eng = factor_lj * phi * scale;
...
double PairTableSWLJCutCoulLong::single_tab_ener(int i, int j, int itype, int jtype, double rsq, double factor_lj)
{
  int itable;
  double fraction,value,a,b,phi;
  int tlm1 = tablength - 1;
...
double PairTableSWLJCutCoulLong::single_tab_fpair(int i, int j, int itype, int jtype, double rsq, double factor_lj)
{
  int itable;
  double fraction,value,a,b,phi;
  int tlm1 = tablength - 1;
