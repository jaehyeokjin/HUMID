...
PPPM_Electrode::PPPM_Electrode(LAMMPS *lmp, int narg, char **arg) : EVB_KSpace(lmp, narg, arg)
{
  ...
  /* Electrode Model */
  img1_rl = img1_im = img2_rl = img2_im = NULL;
  work1_img = NULL;
  greensfn_rev = NULL;
  /* End */
}
...
void PPPM_Electrode::init()
{
  ...
  if( strcmp(force->pair_style,"electrode") && strcmp(force->pair_style,"electrode/omp") )
    error->all(FLERR, "Incompatible pair-style is used.(force->pair_style != electrode)");

  pair_et = (PairElectrode*)(force->pair);
...
void PPPM_Electrode::setup_triclinic()
{
  int i,j,k,n;
  double *prd;
...
void PPPM_Electrode::setup_grid()
{
  // free all arrays previously allocated
...
void PPPM_Electrode::compute(int eflag, int vflag)
{ 
  int i,j;
...
void PPPM_Electrode::allocate()
{
  ...
  /* Eletrode Model */
  memory->create(img1_rl,nfft_both,"pppm:img1_rl");
  memory->create(img1_im,nfft_both,"pppm:img1_im");
  memory->create(img2_rl,nfft_both,"pppm:img2_rl");
  memory->create(img2_im,nfft_both,"pppm:img2_im");
  memory->create(work1_img, 2*nfft_both,"pppm:work1_img1");
  memory->create(greensfn_rev, nfft_both,"pppm:greensfn_rev");
  /* End */		      
}
...
void PPPM_Electrode::deallocate()
{
  ...
  /* Eletrode Model */
  memory->destroy(img1_rl);
  memory->destroy(img1_im);
  memory->destroy(img2_rl);
  memory->destroy(img2_im);
  memory->destroy(work1_img);
  memory->destroy(greensfn_rev);
  /* End */
}
...
void PPPM_Electrode::allocate_peratom()
{
...
void PPPM_Electrode::deallocate_peratom()
{
  peratom_allocate_flag = 0;
...
void PPPM_Electrode::set_grid_global()
{
...
int PPPM_Electrode::factorable(int n)
{
  int i;
...
double PPPM_Electrode::compute_df_kspace()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
double PPPM_Electrode::compute_qopt()
{
  double qopt = 0.0;
...
double PPPM_Electrode::estimate_ik_error(double h, double prd, bigint natoms)
{
  double sum = 0.0;
...
void PPPM_Electrode::adjust_gewald()
{
  double dx;
...
double PPPM_Electrode::newton_raphson_f()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
double PPPM_Electrode::final_accuracy()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
void PPPM_Electrode::set_grid_local()
{
...
/* Electrode Model */
  nzlo_in = static_cast<int>
      (comm->zsplit[comm->myloc[2]] * nz_pppm/(slab_volfactor*pair_et->D * 3/domain->prd[2]));
  nzhi_in = static_cast<int>
      (comm->zsplit[comm->myloc[2]+1] * nz_pppm/(slab_volfactor*pair_et->D * 3/domain->prd[2])) - 1;
/* End */
...
void PPPM_Electrode::compute_gf_denom()
{
  int k,l,m;
...
void PPPM_Electrode::compute_gf_ik()
{
  ...
  /* Eletrode Model */
  double dz1 = - 2.0 * (pair_et->pos_lo - domain->boxlo[2]);
  double dz2 = dz1 - 2.0 * pair_et->D;
  /* End */
...
void PPPM_Electrode::compute_gf_ik_triclinic()
{
  double snx,sny,snz;
  double argx,argy,argz,wx,wy,wz,sx,sy,sz,qx,qy,qz;
  double sum1,dot1,dot2;
...
void PPPM_Electrode::compute_gf_ad()
{
  const double * const prd = domain->prd;
...
void PPPM_Electrode::compute_sf_precoeff()
{
  int i,k,l,m,n;
...
void PPPM_Electrode::particle_map()
{
  int nx,ny,nz;
...
void PPPM_Electrode::poisson_ik()
{
  int i,j,k,n;
  double eng;
...
void PPPM_Electrode::poisson_ik_triclinic()
{
  int i,j,k,n;
...
void PPPM_Electrode::poisson_ad()
{
  int i,j,k,n;
  double eng;
...
void PPPM_Electrode::poisson_peratom()
{
  int i,j,k,n;
...
void PPPM_Electrode::fieldforce()
{
...
void PPPM_Electrode::fieldforce_ik()
{
...
void PPPM_Electrode::fieldforce_ad()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz;
  FFT_SCALAR ekx,eky,ekz;
...
void PPPM_Electrode::fieldforce_peratom()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void PPPM_Electrode::pack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void PPPM_Electrode::unpack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void PPPM_Electrode::pack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPM_Electrode::unpack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPM_Electrode::procs2grid2d(int nprocs, int nx, int ny, int *px, int *py)
{
...
void PPPM_Electrode::compute_rho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                         const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPM_Electrode::compute_drho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                          const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPM_Electrode::compute_rho_coeff()
{
  int j,k,l,m;
  FFT_SCALAR s;
...
int PPPM_Electrode::timing_1d(int n, double &time1d)
{
  double time1,time2;
...
int PPPM_Electrode::timing_3d(int n, double &time3d)
{
  double time1,time2;
...
double PPPM_Electrode::memory_usage()
{
...
/* ----------------------------------------------------------------------
   compute the PPPM_Electrode total long-range force and energy for groups A and B
 ------------------------------------------------------------------------- */

void PPPM_Electrode::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void PPPM_Electrode::allocate_groups()
{
...
void PPPM_Electrode::deallocate_groups()
{
  group_allocate_flag = 0;
...
void PPPM_Electrode::make_rho_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  int l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void PPPM_Electrode::poisson_groups(int AA_flag)
{
  int i,j,k,n;
...
void PPPM_Electrode::poisson_groups_triclinic()
{
  int i,j,k,n;
...
void PPPM_Electrode::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
...
void PPPM_Electrode::compute_cplx(int vflag) { compute(1,vflag); energy /= comm->nprocs; }

void PPPM_Electrode::compute_exch(int vflag)
{
  off_diag_energy = 0.0;
  if (vflag) for (int n = 0; n < 6; n++) off_diag_virial[n] = 0.0;
  
  pair_et->A_Rq = A_Rq;
  pair_et->is_exch_chg = is_exch_chg;

  double epair = pair_et->compute_exch(vflag);
