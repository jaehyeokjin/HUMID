...
f2 = []
rmax_count = max(r)
r_bin_rough = int(50.0*(rmax_count-0.2) + 0.2)
r=[]
for i in range(0, r_bin_rough):
    rvalue = "%.2f" % (0.2 + 0.02 * float(i))
    r.append(float(rvalue))
r_final = [] # Final r after checking the value < threshold
r_correction_site = 2.26
f_correction_factor = f[r_og.index(r_correction_site)] - (-(func(r[r.index(r_correction_site)], par[0], par[1]) - func(r[r.index(r_correction_site)+1], par[0], par[1]))/(r[r.index(r_correction_site)] - r[r.index(r_correction_site)+1]))

for i in range(len(r)):
    if r[i] < r_small:
        vtmp = func(r[i], par[0], par[1])
        ftmp = -(func(r[i], par[0], par[1]) - func(r[i+1], par[0], par[1]))/(r[i] - r[i+1])+f_correction_factor
        if vtmp < v_threshold:
            r_final.append(r[i])
            v2.append(vtmp)
            f2.append(ftmp)
