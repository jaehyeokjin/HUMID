...
int main(int argc, char *argv[]) {
    ...
    double box_length = 100.00;
    double slab_length = 125.25800;
        ...
        for (size_t i = 0; i < cg_site; ++i) {
            ...
            for (size_t j = 0; j < cg_site; ++j) {
                if (i != j)  {
                    ...
                    local_density_22 += 1.0/(1.0+exp(11.0*(distance-2.65)));
                }
            }
            probability_22[i] = 0.5*(1.0+tanh((local_density_22-1.6)/(0.18)));
        }
        if (evb_number >-1){
            for (size_t i = 0; i < cg_site; ++i) {
                double dx22 = x[i] - x[evb_number];
                double dy22 = y[i] - y[evb_number];
                double dz22 = z[i] - z[evb_number];
                min_image(dx22, box_length);
                min_image(dy22, box_length);
                min_image_slab(dz22, slab_length);
                double distance_squared_22 = dx22 * dx22 + dy22 * dy22 + dz22 * dz22;
                ...
                if (distance < evb_distance){
                    neighbor_id[i]=i;
                    neighbor_prob[i]=probability_22[i];
                }
                else{
                    neighbor_id[i]=-1;
                    neighbor_prob[i]=-1.0;
            ...
            double max_val = 0.0;
            for (size_t i = 0; i<cg_site; ++i){
                if (probability_22[i] > max_val){
                    max_val = probability_22[i];
                    max_id = i;
                }
            }
            type[max_id] = 2;
