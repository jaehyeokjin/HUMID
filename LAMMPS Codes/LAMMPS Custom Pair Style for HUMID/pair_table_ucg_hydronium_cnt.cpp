...
PairTable_UCG_HYDRONIUM_CNT::PairTable_UCG_HYDRONIUM_CNT(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_UCG_HYDRONIUM_CNT_NEIGH
  nmax = 0;
...
PairTable_UCG_HYDRONIUM_CNT::~PairTable_UCG_HYDRONIUM_CNT()
{
...
void PairTable_UCG_HYDRONIUM_CNT::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 2){
    double tanh_factor = tanh((cv - cv_thresholds[type]) / (0.18 * cv_thresholds[type]));
    prob = 0.5 + 0.5 * tanh_factor;
    partial = +1.0 * 0.5 * (1.0 - tanh_factor * tanh_factor) / (0.18 * cv_thresholds[type]);
  }
  else{
...
int PairTable_UCG_HYDRONIUM_CNT::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_UCG_HYDRONIUM_CNT::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_UCG_HYDRONIUM_CNT::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_UCG_HYDRONIUM_CNT::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_UCG_HYDRONIUM_CNT::compute_proximity_function(int type, double distance) {
  double tanh_factor = 1.0/(1.0+exp(11.0*(distance - threshold_radii[type])));
  return tanh_factor;
}

double PairTable_UCG_HYDRONIUM_CNT::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = exp(11.0*(distance - threshold_radii[type]));
  return +11.0*tanh_factor/((1.0+tanh_factor)*(1.0+tanh_factor));
}

/* ---------------------------------------------------------------------- */

void PairTable_UCG_HYDRONIUM_CNT::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/ucg_hydronium_cnt:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/ucg_hydronium_cnt:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/ucg_hydronium_cnt:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/ucg_hydronium_cnt:substate_cv_backforce");
  }
...
void PairTable_UCG_HYDRONIUM_CNT::allocate()
{
  allocated = 1;
...
void PairTable_UCG_HYDRONIUM_CNT::settings(int narg, char **arg)
{
...
void PairTable_UCG_HYDRONIUM_CNT::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_UCG_HYDRONIUM_CNT::coeff(int narg, char **arg)
{
...
void PairTable_UCG_HYDRONIUM_CNT::init_style()
{
...
double PairTable_UCG_HYDRONIUM_CNT::init_one(int i, int j)
{
...
void PairTable_UCG_HYDRONIUM_CNT::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_UCG_HYDRONIUM_CNT::bcast_table(Table *tb)
{
...
void PairTable_UCG_HYDRONIUM_CNT::spline_table(Table *tb)
{
...
void PairTable_UCG_HYDRONIUM_CNT::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_UCG_HYDRONIUM_CNT::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_UCG_HYDRONIUM_CNT::null_table(Table *tb)
{
...
void PairTable_UCG_HYDRONIUM_CNT::free_table(Table *tb)
{
...
void PairTable_UCG_HYDRONIUM_CNT::spline(double *x, double *y, int n,
...
double PairTable_UCG_HYDRONIUM_CNT::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_UCG_HYDRONIUM_CNT::write_restart(FILE *fp)
{
...
void PairTable_UCG_HYDRONIUM_CNT::read_restart(FILE *fp)
{
...
void PairTable_UCG_HYDRONIUM_CNT::write_restart_settings(FILE *fp)
{
...
void PairTable_UCG_HYDRONIUM_CNT::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_UCG_HYDRONIUM_CNT::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_UCG_HYDRONIUM_CNT::extract(const char *str, int &dim)
{
