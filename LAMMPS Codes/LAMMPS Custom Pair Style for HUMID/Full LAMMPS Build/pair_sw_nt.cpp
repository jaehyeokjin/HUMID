...
PairSW_NT::PairSW_NT(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairSW_NT::compute(int eflag, int vflag)
{
  int i,j,k,ii,jj,kk,inum,jnum;
  int itype,jtype,ktype,ijparam,ikparam,ijkparam;
  int ijparam1,ijparam2,ikparam1,ikparam2,ijkparam1,ijkparam2;
  tagint itag,jtag;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair, delx1, dely1, delz1;
  double rsq,rsq1,rsq2,rsq3;
  double rsq11,rsq21,rsq12,rsq22;
  double delr1[3],delr2[3],fj[3],fk[3];
  double delr11[3],delr21[3],fj1[3],fk1[3];
  double delr12[3],delr22[3],fj2[3],fk2[3];
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      j &= NEIGHMASK;
      jtype = map[type[j]];
      ijparam = elem2param[jtype][itype][itype];
      delr1[0] = xtmp - x[j][0];
      delr1[1] = ytmp - x[j][1];
      delr1[2] = ztmp - x[j][2];
      rsq1 = delr1[0]*delr1[0] + delr1[1]*delr1[1] + delr1[2]*delr1[2];
      if (rsq1 > params[ijparam].cutsq * 0.25) continue;
      for (kk = 0; kk < jnum; kk++) {
        k = jlist[kk];
        if (k != j){
          k &= NEIGHMASK;
          ktype = map[type[k]];
          ikparam = elem2param[jtype][ktype][ktype];
          ijkparam = elem2param[jtype][itype][ktype];
          delr2[0] = x[k][0] - x[j][0];
          delr2[1] = x[k][1] - x[j][1];
          delr2[2] = x[k][2] - x[j][2];
          ...
          dely1 = x[k][1] - ytmp;
          delz1 = x[k][2] - ztmp;
          rsq3 = delx1*delx1+dely1*dely1+delz1*delz1;
          if (rsq2 > params[ikparam].cutsq * 0.25) continue; // k has to be j's neighbor
          if ((rsq3 < params[ikparam].cutsq * 0.25) && (jj > kk)) continue; // if k is i's neighbor but turn off the double-counting

          threebody(&params[ijparam],&params[ikparam],&params[ijkparam],
                    rsq1,rsq2,delr1,delr2,fj,fk,eflag,evdwl);
// In this case it is i-j-k
          f[i][0] += fj[0];
          f[i][1] += fj[1];
          f[i][2] += fj[2];

          if ((rsq3 < params[ikparam].cutsq * 0.25) && (jj < kk)){ // this is j,k are both i's neighbor
            delr11[0] = x[j][0] - xtmp;
            delr11[1] = x[j][1] - ytmp;
            delr11[2] = x[j][2] - ztmp;
            rsq11 = delr11[0]*delr11[0] + delr11[1]*delr11[1] + delr11[2]*delr11[2];
            ijparam1 = elem2param[itype][jtype][jtype];
            ikparam1 = elem2param[itype][ktype][ktype];
            ijkparam1 = elem2param[itype][jtype][ktype];

            delr21[0] = x[k][0] - xtmp;
            delr21[1] = x[k][1] - ytmp;
            delr21[2] = x[k][2] - ztmp;
            rsq21 = delr21[0]*delr21[0] + delr21[1]*delr21[1] + delr21[2]*delr21[2];
            threebody(&params[ijparam1],&params[ikparam1],&params[ijkparam1],
                      rsq11,rsq21,delr11,delr21,fj1,fk1,eflag,evdwl);
            f[i][0] -= fj1[0] + fk1[0];
            f[i][1] -= fj1[1] + fk1[1];
            f[i][2] -= fj1[2] + fk1[2];
            if (evflag) ev_tally3(i,j,k,evdwl,0.0,fj1,fk1,delr11,delr21);
            // The last case i-k-j
            delr12[0] = x[j][0] - x[k][0];
            delr12[1] = x[j][1] - x[k][1];
            delr12[2] = x[j][2] - x[k][2];
            rsq12 = delr12[0]*delr12[0] + delr12[1]*delr12[1] + delr12[2]*delr12[2];
            ijparam2 = elem2param[ktype][jtype][jtype];
            ikparam2 = elem2param[ktype][itype][itype];
            ijkparam2 = elem2param[ktype][jtype][itype];

            delr22[0] = xtmp - x[k][0];
            delr22[1] = ytmp - x[k][1];
            delr22[2] = ztmp - x[k][2];
            rsq22 = delr22[0]*delr22[0] + delr22[1]*delr22[1] + delr22[2]*delr22[2];
            threebody(&params[ijparam2],&params[ikparam2],&params[ijkparam2],
                      rsq12,rsq22,delr12,delr22,fj2,fk2,eflag,evdwl);
            f[i][0] += fk2[0];
            f[i][1] += fk2[1];
            f[i][2] += fk2[2];
          }
...
void PairSW_NT::settings(int narg, char **arg)
{
...
void PairSW_NT::coeff(int narg, char **arg)
{
  int i,j,n;
...
double PairSW_NT::init_one(int i, int j)
{
...
void PairSW_NT::read_file(char *file)
{
...
void PairSW_NT::twobody(Param *param, double rsq, double &fforce,
...
void PairSW_NT::threebody(Param *paramij, Param *paramik, Param *paramijk,
