...
PairTable_MSUCG_NEO::PairTable_MSUCG_NEO(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_MSUCG_NEO_NEIGH
  nmax = 0;
...
PairTable_MSUCG_NEO::~PairTable_MSUCG_NEO()
{
...
void PairTable_MSUCG_NEO::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) {
...
int PairTable_MSUCG_NEO::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_MSUCG_NEO::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_MSUCG_NEO::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_MSUCG_NEO::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_MSUCG_NEO::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_MSUCG_NEO::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_MSUCG_NEO::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/msucg_neo:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/msucg_neo:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/msucg_neo:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/msucg_neo:substate_cv_backforce");
  }
...
void PairTable_MSUCG_NEO::allocate()
{
  allocated = 1;
...
void PairTable_MSUCG_NEO::settings(int narg, char **arg)
{
...
void PairTable_MSUCG_NEO::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  if (eof == NULL) error->one(FLERR,"Unexpected end of MSUCG_NEO state settings file");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        ...
        error->one(FLERR,"Unknown state assignment type for MSUCG_NEO");
      }
...
void PairTable_MSUCG_NEO::coeff(int narg, char **arg)
{
...
void PairTable_MSUCG_NEO::init_style()
{
  ...
  */
  ...
  else fprintf(screen, "Pair/MSUCG_NEO Ensemble temperature is %lf.\n", T);
...
double PairTable_MSUCG_NEO::init_one(int i, int j)
{
...
void PairTable_MSUCG_NEO::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_MSUCG_NEO::bcast_table(Table *tb)
{
...
void PairTable_MSUCG_NEO::spline_table(Table *tb)
{
...
void PairTable_MSUCG_NEO::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_MSUCG_NEO::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_MSUCG_NEO::null_table(Table *tb)
{
...
void PairTable_MSUCG_NEO::free_table(Table *tb)
{
...
void PairTable_MSUCG_NEO::spline(double *x, double *y, int n,
...
double PairTable_MSUCG_NEO::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_MSUCG_NEO::write_restart(FILE *fp)
{
...
void PairTable_MSUCG_NEO::read_restart(FILE *fp)
{
...
void PairTable_MSUCG_NEO::write_restart_settings(FILE *fp)
{
...
void PairTable_MSUCG_NEO::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_MSUCG_NEO::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_MSUCG_NEO::extract(const char *str, int &dim)
{
