...
PairTable_com::PairTable_com(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
void PairTable_com::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair,xtmp1,ytmp1,ztmp1;
  double xother,yother,zother,delx1,dely1,delz1, comix, comiy, comiz, xother1, yother1, zother1, delx2, dely2, delz2; // Needed for center of mass
  double boxx, boxy, boxz, comjx, comjy, comjz, dcomx,dcomy,dcomz, distcom, rsqcom; // Needed for wrapping the coordinates
  ...
  boxx = domain->boxhi[0]-domain->boxlo[0];
  boxy = domain->boxhi[1]-domain->boxlo[1];
  boxz = domain->boxhi[2]-domain->boxlo[2];
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    jlist = firstneigh[i];
    jnum = numneigh[i];
//    fprintf(screen,"%d (%lf,%lf,%lf) (%lf,%lf,%lf)\n", atom->tag[i], xtmp,ytmp,ztmp,xother,yother,zother);
//    fprintf(screen,"%d (%lf,%lf,%lf) (%lf,%lf,%lf)\n", atom->tag[i], domain->boxlo[0], domain->boxlo[1], domain->boxlo[2], domain->boxhi[0], domain->boxhi[1], domain->boxhi[2]); 
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      ...
      if (itype != jtype){
        // COM consideration
        if (itype == 1){// in this case it is IcJo case
          xother = x[atom->map(atom->tag[i]+1)][0];
          yother = x[atom->map(atom->tag[i]+1)][1];
          zother = x[atom->map(atom->tag[i]+1)][2];
          delx1 = xother - xtmp; // rC->O vector
          dely1 = yother - ytmp;
          delz1 = zother - ztmp;
          xother1 = x[atom->map(atom->tag[j]-1)][0];
          yother1 = x[atom->map(atom->tag[j]-1)][1];
          zother1 = x[atom->map(atom->tag[j]-1)][2];
          delx2 = xtmp1 - xother1; // rC->O vector
          dely2 = ytmp1 - yother1;
          delz2 = ztmp1 - zother1;
        }
        else{// in this case it is IoJc case
          xother = x[atom->map(atom->tag[i]-1)][0]; //C
          yother = x[atom->map(atom->tag[i]-1)][1];
          zother = x[atom->map(atom->tag[i]-1)][2];
          delx1 = xtmp - xother; // rC->O
          dely1 = ytmp - yother;
          delz1 = ztmp - zother;
          xother1 = x[atom->map(atom->tag[j]+1)][0]; // O
          yother1 = x[atom->map(atom->tag[j]+1)][1];
          zother1 = x[atom->map(atom->tag[j]+1)][2];
          delx2 = xother1 - xtmp1; // rC->O
          dely2 = yother1 - ytmp1;
          delz2 = zother1 - ztmp1;
        }
        // Now wrap the coordinate
        if (delx1 > 0.5 * boxx) delx1 -= boxx; 
        else if (delx1 < -0.5 * boxx) delx1 += boxx;
        if (dely1 > 0.5 * boxy) dely1 -= boxy;
        else if (dely1 < -0.5 * boxy) dely1 += boxy;
        if (delz1 > 0.5 * boxz) delz1 -= boxz;
        else if (delz1 < -0.5 * boxz) delz1 += boxz;
        // double wrap
        if (delx2 > 0.5 * boxx) delx2 -= boxx;
        else if (delx2 < -0.5 * boxx) delx2 += boxx;
        if (dely2 > 0.5 * boxy) dely2 -= boxy;
        else if (dely2 < -0.5 * boxy) dely2 += boxy;
        if (delz2 > 0.5 * boxz) delz2 -= boxz;
        else if (delz2 < -0.5 * boxz) delz2 += boxz;
        // Calculate com vector
        if (itype == 1){// in this case it is IcJo case
          comix = (15.035 * xtmp + 17.007 *(xtmp + delx1)) / (17.007+15.035);
          comiy = (15.035 * ytmp + 17.007 *(ytmp + dely1)) / (17.007+15.035);
          comiz = (15.035 * ztmp + 17.007 *(ztmp + delz1)) / (17.007+15.035);
          comjx = (15.035 * xother1 + 17.007 * (xother1+delx2)) / (17.007+15.035);
          comjy = (15.035 * yother1 + 17.007 * (yother1+dely2)) / (17.007+15.035);
          comjz = (15.035 * zother1 + 17.007 * (zother1+delz2)) / (17.007+15.035);
        }
        else {// in this case it is IoJc case
          comix = (15.035 * xother + 17.007 *(xother + delx1)) / (17.007+15.035);
          comiy = (15.035 * yother + 17.007 *(yother + dely1)) / (17.007+15.035);
          comiz = (15.035 * zother + 17.007 *(zother + delz1)) / (17.007+15.035);
          comjx = (15.035 * x[j][0] + 17.007 * (x[j][0]+delx2)) / (17.007+15.035);
          comjy = (15.035 * x[j][1] + 17.007 * (x[j][1]+dely2)) / (17.007+15.035);
          comjz = (15.035 * x[j][2] + 17.007 * (x[j][2]+delz2)) / (17.007+15.035);
        }
        // Re wrap the com vectors
        if (comix > boxx) comix -= boxx; 
        else if (comix < 0.0) comix += boxx;
        if (comiy > boxy) comiy -= boxy;
        else if (comiy < 0.0) comiy += boxy;
        if (comiz > boxz) comiz -= boxz;
        else if (comiz < 0.0) comiz += boxz;
        if (comjx > boxx) comjx -= boxx;
        else if (comjx < 0.0) comjx += boxx;
        if (comjy > boxy) comjy -= boxy;
        else if (comjy < 0.0) comjy += boxy;
        if (comjz > boxz) comjz -= boxz;
        else if (comjz < 0.0) comjz += boxz;
        // Delta com vector and its distances (and additional wrap)
        dcomx = comix - comjx;
        dcomy = comiy - comjy;
        dcomz = comiz - comjz;
        if (dcomx > 0.5 * boxx) dcomx -= boxx;
        else if (dcomx < -0.5 * boxx) dcomx += boxx;
        if (dcomy > 0.5 * boxy) dcomy -= boxy;
        else if (dcomy < -0.5 * boxy) dcomy += boxy;
        if (dcomz > 0.5 * boxz) dcomz -= boxz;
        else if (dcomz < -0.5 * boxz) dcomz += boxz;
        rsqcom = dcomx*dcomx + dcomy*dcomy + dcomz*dcomz;
        if (rsqcom < cutsq[3][3]) {
          distcom = sqrt(rsqcom);
          tb = &tables[tabindex[3][3]];
          if (rsqcom < tb->innersq)
            error->one(FLERR,"Pair distance < table inner cutoff");
          if (tabstyle == SPLINE) {
            itable = static_cast<int> ((rsqcom - tb->innersq) * tb->invdelta);
            if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
            b = (rsqcom - tb->rsq[itable]) * tb->invdelta;
            a = 1.0 - b;
          ...
          //fprintf(screen, "%d(%d) %d(%d) interact!\n", atom->tag[i], itype, atom->tag[j], jtype);
          if (itype == 1){
            fprintf(screen,"%d (%lf,%lf,%lf) (%lf,%lf,%lf) has (%lf,%lf,%lf)\n >>>>> (%lf,%lf,%lf) (%lf,%lf,%lf) has (%lf,%lf,%lf) \n >>>>> (%lf,%lf,%lf) with %lf\n", atom->tag[i], x[i][0], x[i][1], x[i][2], xother,yother,zother, comix, comiy, comiz,  xother1, yother1, zother1, xtmp1, ytmp1, ztmp1, comjx, comjy, comjz, dcomx, dcomy, dcomz, distcom);
            f[i][0] += fpair*15.035/(15.035+17.007)*dcomx;
            f[i][1] += fpair*15.035/(15.035+17.007)*dcomy;
            f[i][2] += fpair*15.035/(15.035+17.007)*dcomz;
            if (newton_pair || j < nlocal) {
              f[j][0] -= fpair*17.007/(15.035+17.007)*dcomx;
              f[j][1] -= fpair*17.007/(15.035+17.007)*dcomy;
              f[j][2] -= fpair*17.007/(15.035+17.007)*dcomz;
            }
            if (eflag) {
            ...
            if (evflag) ev_tally(i,j,nlocal,newton_pair,
                                 evdwl,0.0,fpair*15.035/(15.035+17.007),dcomx, dcomy, dcomz);
          }
          else{
            f[i][0] += fpair*17.007/(15.035+17.007)*dcomx;
            f[i][1] += fpair*17.007/(15.035+17.007)*dcomy;
            f[i][2] += fpair*17.007/(15.035+17.007)*dcomz;
            if (newton_pair || j < nlocal) {
              f[j][0] -= fpair*15.035/(15.035+17.007)*dcomx;
              f[j][1] -= fpair*15.035/(15.035+17.007)*dcomy;
              f[j][2] -= fpair*15.035/(15.035+17.007)*dcomz;
            }
            if (eflag) {
              evdwl = a * tb->e[itable] + b * tb->e[itable+1] +
                ((a*a*a-a)*tb->e2[itable] + (b*b*b-b)*tb->e2[itable+1]) *
            ...
            if (evflag) ev_tally(i,j,nlocal,newton_pair,
                                 evdwl,0.0,fpair*17.007/(15.035+17.007),dcomx, dcomy, dcomz);
          }
        }
      }
...
void PairTable_com::settings(int narg, char **arg)
{
...
void PairTable_com::coeff(int narg, char **arg)
{
...
double PairTable_com::init_one(int i, int j)
{
...
void PairTable_com::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_com::bcast_table(Table *tb)
{
...
void PairTable_com::spline_table(Table *tb)
{
...
void PairTable_com::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_com::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_com::null_table(Table *tb)
{
...
void PairTable_com::free_table(Table *tb)
{
...
void PairTable_com::spline(double *x, double *y, int n,
...
double PairTable_com::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_com::write_restart(FILE *fp)
{
...
void PairTable_com::read_restart(FILE *fp)
{
...
void PairTable_com::write_restart_settings(FILE *fp)
{
...
void PairTable_com::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_com::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_com::extract(const char *str, int &dim)
{
