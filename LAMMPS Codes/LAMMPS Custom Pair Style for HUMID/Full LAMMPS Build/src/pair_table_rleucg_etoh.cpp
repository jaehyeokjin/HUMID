...
PairTable_RLEUCG_ETOH::PairTable_RLEUCG_ETOH(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_ETOH_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_ETOH::~PairTable_RLEUCG_ETOH()
{
...
void PairTable_RLEUCG_ETOH::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 3){
    double exponential_factor = 0.905528971622684 * exp(-1.48855171927778 * cv);
    prob = (1.0-0.426770572955818) - exponential_factor;
    partial = 1.48855171927778 * exponential_factor;
  }
/*
  else if (type == 1){
...
int PairTable_RLEUCG_ETOH::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_ETOH::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_ETOH::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_ETOH::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_ETOH::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_ETOH::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_ETOH::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_etoh:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_etoh:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_etoh:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_etoh:substate_cv_backforce");
  }
...
void PairTable_RLEUCG_ETOH::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_ETOH::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_ETOH::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_ETOH::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_ETOH::init_style()
{
  ...
  */
  ...
  else fprintf(screen, "pair/rleucg_etoh Ensemble temperature is %lf.\n", T);
...
double PairTable_RLEUCG_ETOH::init_one(int i, int j)
{
...
void PairTable_RLEUCG_ETOH::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_ETOH::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_ETOH::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_ETOH::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_ETOH::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_ETOH::null_table(Table *tb)
{
...
void PairTable_RLEUCG_ETOH::free_table(Table *tb)
{
...
void PairTable_RLEUCG_ETOH::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_ETOH::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_ETOH::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_ETOH::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_ETOH::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_ETOH::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_ETOH::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_ETOH::extract(const char *str, int &dim)
{
