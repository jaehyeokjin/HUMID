...
PairTable_RLEUCG_LL::PairTable_RLEUCG_LL(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_LL_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_LL::~PairTable_RLEUCG_LL()
{
...
void PairTable_RLEUCG_LL::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTable_RLEUCG_LL::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_LL::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_LL::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_LL::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_LL::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_LL::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_LL::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_ll:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_ll:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_ll:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_ll:substate_cv_backforce");
  }
...
void PairTable_RLEUCG_LL::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_LL::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_LL::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_LL::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_LL::init_style()
{
...
double PairTable_RLEUCG_LL::init_one(int i, int j)
{
...
void PairTable_RLEUCG_LL::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_LL::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_LL::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_LL::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_LL::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_LL::null_table(Table *tb)
{
...
void PairTable_RLEUCG_LL::free_table(Table *tb)
{
...
void PairTable_RLEUCG_LL::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_LL::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_LL::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_LL::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_LL::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_LL::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_LL::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_LL::extract(const char *str, int &dim)
{
