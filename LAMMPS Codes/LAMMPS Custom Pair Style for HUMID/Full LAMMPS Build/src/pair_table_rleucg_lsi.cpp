...
PairTable_RLEUCG_LSI::PairTable_RLEUCG_LSI(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_LSI_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_LSI::~PairTable_RLEUCG_LSI()
{
...
void PairTable_RLEUCG_LSI::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTable_RLEUCG_LSI::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_LSI::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_LSI::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_LSI::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_LSI::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_LSI::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_LSI::compute(int eflag, int vflag)
{
...
void PairTable_RLEUCG_LSI::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_LSI::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_LSI::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  if (eof == NULL) error->one(FLERR,"Unexpected end of RLEUCG_LSI state settings file");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        ...
        error->one(FLERR,"Unknown state assignment type for RLEUCG_LSI");
      }
...
void PairTable_RLEUCG_LSI::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_LSI::init_style()
{
  ...
  */
  ...
  else fprintf(screen, "Pair/RLEUCG_LSI Ensemble temperature is %lf.\n", T);
...
double PairTable_RLEUCG_LSI::init_one(int i, int j)
{
...
void PairTable_RLEUCG_LSI::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_LSI::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_LSI::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_LSI::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_LSI::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_LSI::null_table(Table *tb)
{
...
void PairTable_RLEUCG_LSI::free_table(Table *tb)
{
...
void PairTable_RLEUCG_LSI::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_LSI::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_LSI::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_LSI::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_LSI::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_LSI::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_LSI::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_LSI::extract(const char *str, int &dim)
{
