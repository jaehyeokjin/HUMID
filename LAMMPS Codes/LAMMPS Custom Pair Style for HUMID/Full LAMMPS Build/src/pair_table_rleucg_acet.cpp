...
PairTable_RLEUCG_ACET::PairTable_RLEUCG_ACET(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_ACET_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_ACET::~PairTable_RLEUCG_ACET()
{
...
void PairTable_RLEUCG_ACET::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if ((type == 3) || (type == 4)){
...
int PairTable_RLEUCG_ACET::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_ACET::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_ACET::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_ACET::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_ACET::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_ACET::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_ACET::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_acet:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_acet:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_acet:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_acet:substate_cv_backforce");
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (n_states_per_type[itype_actual] > 1) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        jtype = type[j];
        if ((itype == 3 && jtype == 5) || (itype == 5 && jtype == 3)){
          delx = xtmp - x[j][0];
          dely = ytmp - x[j][1];
          delz = ztmp - x[j][2];
...
void PairTable_RLEUCG_ACET::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_ACET::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_ACET::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_ACET::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_ACET::init_style()
{
...
/*
  ...
  else fprintf(screen, "pair/rleucg_acet Ensemble temperature is %lf.\n", T);
*/
  kT = force->boltz * T;
...
double PairTable_RLEUCG_ACET::init_one(int i, int j)
{
...
void PairTable_RLEUCG_ACET::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_ACET::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_ACET::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_ACET::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_ACET::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_ACET::null_table(Table *tb)
{
...
void PairTable_RLEUCG_ACET::free_table(Table *tb)
{
...
void PairTable_RLEUCG_ACET::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_ACET::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_ACET::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_ACET::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_ACET::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_ACET::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_ACET::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_ACET::extract(const char *str, int &dim)
{
