...
PairTable_RLEUCG_DIST::PairTable_RLEUCG_DIST(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  // From PAIR_RLEUCG_DIST_NEIGH
  nmax = 0;
...
PairTable_RLEUCG_DIST::~PairTable_RLEUCG_DIST()
{
...
void PairTable_RLEUCG_DIST::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial) 
{
  if (type == 1){
...
int PairTable_RLEUCG_DIST::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_RLEUCG_DIST::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_RLEUCG_DIST::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
...
void PairTable_RLEUCG_DIST::unpack_forward_comm(int n, int first, double *buf)
{
...
double PairTable_RLEUCG_DIST::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_RLEUCG_DIST::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_RLEUCG_DIST::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  int itype_actual, jtype_actual;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,rsq_a,rsq_b,factor_lj,fraction,value,a,b;
  double jj1,jj2,j1,j2,delx1,delx2,dely1,dely2,delz1,delz2,jtype1,jtype2;
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/rleucg_dist:substate_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/rleucg_dist:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/rleucg_dist:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/rleucg_dist:substate_cv_backforce");
  }
...
void PairTable_RLEUCG_DIST::allocate()
{
  allocated = 1;
...
void PairTable_RLEUCG_DIST::settings(int narg, char **arg)
{
...
void PairTable_RLEUCG_DIST::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
...
void PairTable_RLEUCG_DIST::coeff(int narg, char **arg)
{
...
void PairTable_RLEUCG_DIST::init_style()
{
...
double PairTable_RLEUCG_DIST::init_one(int i, int j)
{
...
void PairTable_RLEUCG_DIST::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_RLEUCG_DIST::bcast_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST::spline_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_RLEUCG_DIST::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_RLEUCG_DIST::null_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST::free_table(Table *tb)
{
...
void PairTable_RLEUCG_DIST::spline(double *x, double *y, int n,
...
double PairTable_RLEUCG_DIST::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_RLEUCG_DIST::write_restart(FILE *fp)
{
...
void PairTable_RLEUCG_DIST::read_restart(FILE *fp)
{
...
void PairTable_RLEUCG_DIST::write_restart_settings(FILE *fp)
{
...
void PairTable_RLEUCG_DIST::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_RLEUCG_DIST::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_RLEUCG_DIST::extract(const char *str, int &dim)
{
