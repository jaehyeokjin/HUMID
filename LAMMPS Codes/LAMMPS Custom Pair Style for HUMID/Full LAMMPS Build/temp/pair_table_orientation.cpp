...
#include "string.h"
#include "math.h"
#include "pair_table_orientation.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
#include "neighbor.h"
...
PairTable_Orientation::PairTable_Orientation(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  nmax = 0;
...
PairTable_Orientation::~PairTable_Orientation()
{
...
void PairTable_Orientation::threshold_prob_and_partial_from_cv(int type, double cv, double &prob, double &partial, double &orient) 
{
  if (type == 1){
  ...
  */
    ...
    error->one(FLERR,"Declared type in Orientation does not exist.");
  }
}

/* ---------------------------------------------------------------------- */

int PairTable_Orientation::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairTable_Orientation::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
int PairTable_Orientation::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m,jsubstate;
  ...
  for (i = 0; i < n; i++) {
    ...
    for (jsubstate = 0; jsubstate < max_states_per_type - 1; jsubstate++) {
      ...
      buf[m++] = orientation_probability[j][jsubstate];
...
void PairTable_Orientation::unpack_forward_comm(int n, int first, double *buf)
{
  ...
  for (i = first; i < last; i++) {
    for (isubstate = 0; isubstate < max_states_per_type - 1; isubstate++) {
      ...
      orientation_probability[i][isubstate] = buf[m++];
...
double PairTable_Orientation::compute_proximity_function(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
  return 0.5 * (1.0 - tanh_factor);
}

double PairTable_Orientation::compute_proximity_function_der(int type, double distance) {
  double tanh_factor = tanh((distance - threshold_radii[type]) / (0.1 * threshold_radii[type]));
...
void PairTable_Orientation::compute(int eflag, int vflag)
{
  ...
  if (nall > nmax) {
    nmax = nall;
    memory->grow(substate_probability, nall, max_states_per_type - 1, "pair/orientation:substate_probability");
    memory->grow(orientation_probability, nall, max_states_per_type - 1, "pair/orientation:orientation_probability");
    memory->grow(substate_probability_partial, nall, max_states_per_type - 1, "pair/orientation:substate_probability_partial");
    memory->grow(substate_probability_force, nall, max_states_per_type - 1, "pair/orientation:substate_probability_force");
    memory->grow(substate_cv_backforce, nall, 3, "pair/orientation:substate_cv_backforce");
  }

  for(i = 0; i < nall; i++) {
    for (j = 0; j < max_states_per_type - 1; j++) {
      substate_probability[i][j] = 0.0;
      orientation_probability[i][j] = 0.0;
...
void PairTable_Orientation::allocate()
{
  allocated = 1;
...
void PairTable_Orientation::settings(int narg, char **arg)
{
...
void PairTable_Orientation::read_state_settings(const char *file) {
  char *eof;
  char line[MAXLINE];
  ...
  if (eof == NULL) error->one(FLERR,"Unexpected end of Orientation state settings file");
  ...
  for (int i = 1; i <= n_actual_types; i++) {
    ...
    if (n_states_per_type[i] > 1) {
      ...
      if (strcmp(state_type, "density") == 0) {
        ...
        error->one(FLERR,"Unknown state assignment type for Orientation");
      }
...
void PairTable_Orientation::coeff(int narg, char **arg)
{
...
void PairTable_Orientation::init_style()
{
...
double PairTable_Orientation::init_one(int i, int j)
{
...
------------------------------------------------------------------------- */
void PairTable_Orientation::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTable_Orientation::bcast_table(Table *tb)
{
...
void PairTable_Orientation::spline_table(Table *tb)
{
...
void PairTable_Orientation::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable_Orientation::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTable_Orientation::null_table(Table *tb)
{
...
void PairTable_Orientation::free_table(Table *tb)
{
...
void PairTable_Orientation::spline(double *x, double *y, int n,
...
double PairTable_Orientation::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable_Orientation::write_restart(FILE *fp)
{
...
void PairTable_Orientation::read_restart(FILE *fp)
{
...
void PairTable_Orientation::write_restart_settings(FILE *fp)
{
...
void PairTable_Orientation::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable_Orientation::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTable_Orientation::extract(const char *str, int &dim)
{
